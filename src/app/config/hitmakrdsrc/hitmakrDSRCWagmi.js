...
export const usePurchaseDSRC = (contractAddress) => {
    ...
    const handleNetworkSwitch = async () => {
        try {
            await switchChain({ chainId: skaleCalypso.id });
            return true;
        } catch (error) {
    ...
    const purchase = async (edition) => {
        ...
        try {
            ...
            if (price > 0n) {
                ...
                if (balance < price) {
                    window.open('https://portal.skale.space/bridge?from=mainnet&to=elated-tan-skat&token=usdc&type=erc20', '_blank');
    ...
    return {
        ...
        isCorrectChain: chainId === skaleCalypso.id,
        isNetworkSwitching,
    };
};
